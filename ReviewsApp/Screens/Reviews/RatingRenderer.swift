...
extension RatingRenderer {
    ...
    func ratingImage(_ rating: Int) -> UIImage {
        if let cached = imageCache.object(forKey: NSNumber(value: rating)) {
            return cached
        }
        let rendered = drawRatingImage(rating)
        imageCache.setObject(rendered, forKey: NSNumber(value: rating))
        return rendered
    }
